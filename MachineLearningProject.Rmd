...
```{r}
db_train %>% 
  bind_cols(predict(log_all_var, type = "prob")) %>% 
  ggplot(aes(d=ifelse(Outcome=="0",0,1), m = `1`)) +
  geom_roc(labelround = 2, size = 1,
           linealpha = .5, pointalpha = .8) +
  geom_abline(slope = 1, intercept = 0, color = "gray")
...
```{r}
db_train %>% 
  bind_cols(predict(log_lasso, type = "prob")) %>% 
  ggplot(aes(d=ifelse(grant_status=="0",0,1), m = `1`)) +
  geom_roc(labelround = 2, size = 1,
           linealpha = .5, pointalpha = .8) +
  geom_abline(slope = 1, intercept = 0, color = "gray")
