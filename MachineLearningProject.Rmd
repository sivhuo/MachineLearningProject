...
From the summary table above, we can see that there are some NA's in Glucose, BloodPressure, and BMI, but there are hundred of NA's in SkinThickness and Insulin. Therefore, we decide to change NA's to the median value for Glucose, BloodPressure, and BMI, and make SkinThickness and Insulin categorical.
...
First of all, we choose four variable out of all the available variables, and use them to create a logistic model. We decided to choose age, BMI, glucose, and Pregnancies.
...
The cross-validated accuracy is 0.7844 for the four-variable model.
...
We then used the confusion matrix to further evaluate the model. From the confusion matrix, we can see that the model accuracy is 0.7881.
...
Then, we plotted the ROC curve and computed the AUC for this model, which is 0.8320.
...
The cross-validated accuracy is 0.7657 for the all-variable model.
...
Similarly, we created a confusion matrix to see the model performance. From the matrix, we can see that the model has an accuracy of 0.7788.
...
Now, we draw a ROC curve and compute the AUC for this model. The AUC for this all-var model is 0.8479.
...
The cross-validated accuracy is 0.7657 for the lasso model.
...
As same as we did before, we created a consusion matrix for our Lasso model, and got an accuracy of 0.7788.
...
After drawing the ROC curve and computed the AUC, we found an AUC of 0.8469 for our Lasso model. 
...
The best tuning paramter is 0.01526418 and the highest accuracy is 0.7492. 
...
The accuracy using the entire training dataset is 0.8290 which is significantly greater than the cross-validated accuracy 0.7492. This could be because the model is overfitted since we use the entire traning data set. 
...
We also use ROC and AUC method to evaluate our model. We want our classifier to perform better than chance where the AUC is 0.5. From the graph above, we can see that it is doing a pretty good job since the ROC curve is above the linear line and the area under it is 0.8601 which is greater than 0.5. 
...
The results table shows the accuracy for each mtry. The highest accuracy is 0.7881 and the best mtry is 2 (best tune). This oob accuracy is greater than the accuracy from the classification tree model. 
...
# Summary-train
    ...
    -----------|-------------|-------------|-------------|-------------|-------------
    Accuracy   |0.7844[4]    |0.7657[2]    |0.7657[2]    |0.7492[1]    |0.7881[5] 
    AUC        |0.8320[2]    |0.8479[4]    |0.8469[3]    |0.8601[5]    |0.8266[1]  

Baed on the summary table above, it is hard to conclude which model is the best. The model with the highest accuracy has the smallest AUC while the one with the highest AUC has the lowest accuracy. Therefore, it is the best to choose the model with the median accuracy and AUC. The all-var is the best choice as it has the second highest AUC and a median accuracy.
...
Both the training summary table and the testing one give to the same choice- the all-var model. As a result, our conclusion is that we should include all variables in the model when examing whether a patient has diabetes or not.